"""
VariationWrapper — Domain randomization para entornos DMControl.

En cada reset() randomiza:
  - Colores RGBA de los geoms del agente       (intensidad variable)
  - Color RGBA del suelo (floor/ground geom)   (variación suave)
  - Dirección e intensidad de la luz           (opcional)

Uso en _make_single_env:
    env = TransformedEnv(
        dmc_env,
        Compose(
            DMControlColorRandomizer(seed=seed),
            ToTensorImage(...),
            Resize(...),
        )
    )
...
# Geoms de "suelo" comunes a varios dominios DMControl
