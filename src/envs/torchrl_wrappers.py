...
def _make_single_env(task: str, img_size: int = 64):
    """
    ...
    if is_dmc:
        ...
        # Action Repeat = 2 recomendado para dmc. Y pixels_only=False por si la red usa estado interno temporalmente.
        env = DMControlEnv(
            env_name=domain,
            task_name=task_name,
            from_pixels=True,
...
def make_torchrl_env(task: str, num_envs: int = 16, seed: int = 42, img_size: int = 64) -> ParallelEnv:
    """
    ...
    create_env_fn = EnvCreator(lambda: _make_single_env(task, img_size=img_size))
